...
class RandomSiameseDataset(Dataset):
    def __init__(self, data, labels, length=1000000):
        self.length = length
        self.loaded_data = data
        self.loaded_labels = labels
        # Take a random subset of 1000 000 wins and losses to obtain 1M input pairs
        self.loaded_wins = self.loaded_data[self.loaded_labels == 1]
        self.loaded_wins = self.loaded_wins[np.random.choice(self.loaded_wins.shape[0], self.length, replace=False), :]
        self.loaded_losses = self.loaded_data[self.loaded_labels == -1]
        self.loaded_losses = self.loaded_losses[np.random.choice(self.loaded_losses.shape[0], self.length, replace=False), :]
