...
def train(args):
    ...
    train_dataset = UpdatedSiameseDataset(
        mode="train",  length=2000000, chunk_size=100000, labels_size=2000000)

    val_dataset = UpdatedSiameseDataset(
        mode="valid",  length=250000, chunk_size=12500, labels_size=250000)
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True)
    valid_loader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=True)

    siamese_net = SmallerSiamese().to(device)
    ...
    for epoch in range(args.epochs):
        ...
        np.savetxt("NeuralNetwork/Results/SmallerYed/{}_Train_batch_res.csv".format(epoch), np.array(loss_train))
        np.savetxt("NeuralNetwork/Results/SmallerYed/{}_Valid_batch_results.csv".format(epoch), np.array(loss_val))
        np.savetxt("NeuralNetwork/Results/SmallerYed/{}_Valid_Res_per_epoch.csv".format(epoch), np.array(valid_epoch))
        np.savetxt("NeuralNetwork/Results/SmallerYed/{}_Train_Res_per_epoch.csv".format(epoch), np.array(train_epoch))
...
def save_checkpoint(model, optim, epoch, args):
    state = {'state_dict': model.state_dict(),
             ...
             'epoch': epoch + 1}
    path_to_file = 'NeuralNetwork/Checkpoints/SmallerYed/lr_{}_decay_{}'.format(int(args.lr * 1000),
                                                                                int(args.decay * 100))
    if not os.path.exists(path_to_file):
        os.makedirs(path_to_file)
    torch.save(state, os.path.join(path_to_file, 'small_siamese_{}.pth.tar'.format(epoch)))
